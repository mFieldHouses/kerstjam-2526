...
func _init():
	# Handige defaults (zodat je node niet “stuk” start)
	set_input_port_default_value(2, 1.0) # scale
	set_input_port_default_value(3, 0.0) # time
...
func _get_code(input_vars, output_vars, mode, type):
	# P = vec3((uv + offset) * scale, time)
	# Remap van [-1..1] naar [0..1]
	return "%s = cnoise(vec3((%s + %s) * %s, %s)) * 0.5 + 0.5;" % [
		output_vars[0],
		input_vars[0],
		input_vars[1],
		input_vars[2],
