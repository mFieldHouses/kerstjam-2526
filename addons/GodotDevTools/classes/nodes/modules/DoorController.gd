...
#@export_enum("Open-Close", "Single", "Armature Actions", "Rotate") var animation_mode : int = 0 ##The way that the animations are controlled.[br][br] - [param Open-Close] assumes that all animations are baked into 1 animation (by setting Split by Object in export settings to false) and that the opening and closing animations are one action and succeed each other within that action. [br] - [param Single] assumes that you only have one animation with one or more objects baked into the scene that you want to play in reverse for the closing animation.[br] - [param Armature Action] assumes all objects are parented to an armature, which then has seperate actions for both opening and closing.
...
@export var open_rotation_offset : float = 90.0 ##The amount of degrees the door will turn from its starting position when the door is opened.
@export var rotation_time : float = 0.4 ##The time it takes for the door to complete its turning animation
