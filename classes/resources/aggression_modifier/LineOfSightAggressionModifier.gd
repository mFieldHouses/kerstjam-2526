...
func get_aggression_factor(entity : Enemy) -> float:
	...
	return float(entity.global_basis.z.angle_to(_entity_to_player) / PI * 180 < entity.behavior_configuration.field_of_view) * float(RaycastManager.is_ray_free(entity.global_position + Vector3(0.0, 0.5, 0.0), PlayerState.player_instance.global_position, 1, true))
